...
static gboolean
fu_synaptics_rmi_v7_device_erase_all(FuSynapticsRmiDevice *self,
				     FuFirmware *firmware,
				     GError **error)
{
	...
	FuSynapticsRmiFunction *f34;
	g_autoptr(GByteArray) erase_cmd = g_byte_array_new();
	g_autoptr(GPtrArray) imgs = fu_synaptics_rmi_firmware_get_images(firmware);
	...
	for (guint i = 0; i < imgs->len; i++) {
		g_autoptr(GPtrArray) bytes_payload = NULL;
		FuFirmware *img = g_ptr_array_index(imgs, i);
		const gchar *id = fu_firmware_get_id(img);

		if (g_strcmp0(id, "ui") == 0) {
		...
		if (g_strcmp0(id, "config") == 0) {
			if (!fu_synaptics_rmi_v7_device_erase_partition(
				self,
				RMI_PARTITION_ID_CORE_CONFIG,
				&error_local)) {
				if (!g_error_matches(error_local,
		...
		if (g_strcmp0(id, "display-config") == 0) {
			if (!fu_synaptics_rmi_v7_device_erase_partition(
				self,
				RMI_PARTITION_ID_DISPLAY_CONFIG,
				&error_local)) {
		...
		if (g_strcmp0(id, "flash-config") == 0) {
			if (!fu_synaptics_rmi_v7_device_erase_partition(
				self,
				RMI_PARTITION_ID_FLASH_CONFIG,
				&error_local)) {
					   ...
					   GError **error)
{
	...
	/* write partition */
	chunks =
	    fu_chunk_array_new_from_bytes(g_ptr_array_index(bytes, 0),
					  0x00, /* start addr */
					  0x00, /* page_sz */
	...
	if (g_ptr_array_index(bytes, 1)) {
		...
		chunks = fu_chunk_array_new_from_bytes(g_ptr_array_index(bytes, 1),
						       0x00, /* start addr */
						       0x00, /* page_sz */
					  ...
					  GError **error)
{
	FuSynapticsRmiDevice *self = FU_SYNAPTICS_RMI_DEVICE(device);
	FuSynapticsRmiFunction *f34;
	g_autoptr(GPtrArray) bytes_bin = NULL;
	g_autoptr(GPtrArray) bytes_cfg = NULL;
	g_autoptr(GPtrArray) bytes_flashcfg = NULL;
	g_autoptr(GPtrArray) bytes_fld = NULL;
	g_autoptr(GPtrArray) bytes_afe = NULL;
	g_autoptr(GPtrArray) bytes_displaycfg = NULL;
	...
	fu_progress_add_step(progress, FWUPD_STATUS_DEVICE_ERASE, 35, NULL);
	fu_progress_add_step(progress, FWUPD_STATUS_DEVICE_WRITE, 1, "flash-config");
	fu_progress_add_step(progress, FWUPD_STATUS_DEVICE_WRITE, 63, "core-code");
	fu_progress_add_step(progress, FWUPD_STATUS_DEVICE_WRITE, 1, "core-config");
	fu_progress_add_step(progress, FWUPD_STATUS_DEVICE_WRITE, 0, "afe");
	...
	bytes_bin = fu_synaptics_rmi_firmware_get_image_by_id(firmware, "ui");
	bytes_cfg = fu_synaptics_rmi_firmware_get_image_by_id(firmware, "config");
	bytes_flashcfg = fu_synaptics_rmi_firmware_get_image_by_id(firmware, "flash-config");
	bytes_fld = fu_synaptics_rmi_firmware_get_image_by_id(firmware, "fld");
	bytes_afe = fu_synaptics_rmi_firmware_get_image_by_id(firmware, "afe");
	bytes_displaycfg = fu_synaptics_rmi_firmware_get_image_by_id(firmware, "display-config");
	...
	/* erase all */
	if (!fu_synaptics_rmi_v7_device_erase_all(self, firmware, error)) {
	...
	if (g_ptr_array_index(bytes_flashcfg, 0) != NULL) {
	...
	/* write core code */
	if (g_ptr_array_index(bytes_bin, 0) != NULL) {
		if (!fu_synaptics_rmi_v7_device_write_partition(self,
								RMI_PARTITION_ID_CORE_CODE,
								bytes_bin,
								fu_progress_get_child(progress),
	...
	/* write core config */
	if (g_ptr_array_index(bytes_cfg, 0) != NULL) {
		if (!fu_synaptics_rmi_v7_device_write_partition(self,
								RMI_PARTITION_ID_CORE_CONFIG,
								bytes_cfg,
								fu_progress_get_child(progress),
	...
	/* write afe */
	if (g_ptr_array_index(bytes_afe, 0) != NULL) {
		if (!fu_synaptics_rmi_v7_device_write_partition(
			self,
			RMI_PARTITION_ID_EXTERNAL_TOUCH_AFE_CONFIG,
			bytes_cfg,
	...
	if (g_ptr_array_index(bytes_displaycfg, 0) != NULL) {
		if (!fu_synaptics_rmi_v7_device_write_partition(self,
								RMI_PARTITION_ID_DISPLAY_CONFIG,
								bytes_cfg,
								fu_progress_get_child(progress),
					...
					GError **error)
{
	...
	if (flash->bootloader_id[0] < 10 && flash->has_pubkey) {
		imgs = fu_synaptics_rmi_firmware_get_images(firmware);
		pubkey = fu_synaptics_rmi_v7_device_get_pubkey(self, error);
		for (guint i = 0; i < imgs->len; i++) {
			g_autoptr(GPtrArray) bytes_payload = NULL;
			FuFirmware *img = g_ptr_array_index(imgs, i);
			const gchar *id = fu_firmware_get_id(img);
			bytes_payload = fu_synaptics_rmi_firmware_get_image_by_id(firmware, id);
			if (g_ptr_array_index(bytes_payload, 1)) {
				verify_result = fu_synaptics_verify_sha256_signature(
				    g_ptr_array_index(bytes_payload, 0),
				    pubkey,
				    g_ptr_array_index(bytes_payload, 1),
				    error);
				if (verify_result < 0) {
					g_prefix_error(error, "%s secure check failed. ", id);
					return FALSE;
				}
				g_debug("%s signature verified successfully \n", id);
			}
		}
	}
	return TRUE;
